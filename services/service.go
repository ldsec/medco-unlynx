...
func SplitNodeSecrets(rootSk kyber.Scalar, rootEphemeral []kyber.Scalar, nbrSplits int) ([]kyber.Scalar, []kyber.Point, [][]kyber.Scalar) {
	sks := SplitScalar(rootSk.Clone(), nbrSplits)
	...
	for _, ep := range rootEphemeral {
		newEps := SplitScalar(ep.Clone(), nbrSplits)
...
func SumEp(ep [][]kyber.Scalar) kyber.Scalar {
	acum := ep[0][0].Clone()
	for i:=0; i<len(ep); i++{
		for j:=0; j<len(ep[i]); j++ {
			if i==0 && j==0 {
				continue
			} else {
				acum = acum.Add(acum, ep[i][j])
			}
		}
	}
	return acum
...
func SplitScalar(rootScalar kyber.Scalar, nbrSplits int) []kyber.Scalar {
	...
	for i:=0; i<nbrSplits; i++ {
		newScalar := libunlynx.RandomScalarSlice(1)[0]
